...
运营工作台把默认信息架构收敛为“创建内容 → 审核内容 → 准备素材 → 发布”，总览根据真实业务状态给出一个建议下一步；知识、渠道、复盘、队列和管理收纳在“资源与系统”，移动端使用原生更多选择器。发布页默认立即执行，也可切换定时；常用字段优先展示，脚本与人工导出放入高级方式；列表区分失败阶段、安全重试与人工对账。活动页可维护结构化 Brief、启动生成、归档与恢复，并按需展开最近 5 次生成记录查看模型与 Prompt 追溯证据；审核页同时展示规则结果、已通过内容和每次模型生成/人工修改的版本历史；复盘页只对官方 API 任务自动回收，其余方式人工录入已核对指标。顶部可切换已授权工作区，管理页可创建隔离工作区、调整成员角色、查看 Prompt/Eval 治理与脱敏审计记录。各业务页依据 `viewer/editor/reviewer/admin` 隐藏或禁用越权操作。界面采用高密度、方形、单一蓝色交互色；80–180ms 动效仅反馈点击、视图进入和消息状态，并遵循 `prefers-reduced-motion`。
