...
- [x] 七类高增长运营列表使用有界 keyset cursor、服务端同步水位、带时区增量过滤和 PostgreSQL 组合索引；Web 初次有界追页并只对运营状态做增量轮询，超出客户端上限会显式提示而非静默截断。
...
- [x] 运营、管理和父级可增长列表均有服务端页长上限与稳定继续游标；Prompt/Eval 摘要有固定上限并提供独立历史端点。
- [x] 单次脚本发布证据具有数量与累计字节上限；单内容版本素材具有数量上限和可索引版本列，旧版本 Worker 任务不会继续调用媒体 Provider。
- [ ] Web 仍需提供超过 2000 条后的按需历史浏览、服务端筛选/导出和虚拟列表；并拆分领域 query hooks/components，用浏览器 E2E、查询预算和负载基线签收大数据量行为。
- [x] 建立工作区总存储账本、并发预留、可重试删除、自动只读巡检、异常指标和版本化告警。
- [ ] 建立业务对象保留期、归档、合法删除/Legal Hold、备份删除传播、全对象哈希抽检与 S3 历史版本成本治理；候选元数据还需独立总字节边界。
...
- [ ] 按[公网测试部署实现计划](public_test_deployment_plan.md)完成固定出口 IP、同源 HTTPS 网关、受控 GHCR 镜像部署、关闭注册、联合备份恢复、版本回滚和微信公众号跨网络草稿验收；规划记录本身不算目标环境签收。
- [x] 仓库内已提供公网专用 Compose/Caddy、静态安全门禁、独立 Embedding API 后备、固定 BGE 缓存、R2 操作探针、离线管理员引导、restic 加密备份以及 GHCR/受保护 SSH 工作流；这些实现不替代上一项的目标云环境证据。
