...
- [x] AI 生成追溯：工作流保存 Provider/模型、Prompt 集版本与模板哈希、分阶段调用、摘要、时延和 Provider 返回的 Token 用量；失败证据可追踪且不复制原始 Prompt/正文，不虚构成本。
...
- [x] 后端 73 项测试通过、6 项外部服务集成测试在本机跳过；真实 PostgreSQL/pgvector 集成测试 3 项、MinIO 集成测试 2 项、新增 1 项 PostgreSQL 限流并发门禁待 CI 执行，分支覆盖率 78.42%，Ruff、锁文件一致性和 `pip-audit` 均通过。
...
- [ ] 持久 PostgreSQL 仍停留在上一 head `c9e7b4a2d610`；Docker 引擎恢复后必须先使用现有回滚点，经 `f4c2d8e7a190` 迁移到 `a73f9c2e4b61`，并重跑 21 表 PostgreSQL+MinIO 联合恢复。
