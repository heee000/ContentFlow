...
- [x] 总览、活动创建/编辑/归档、知识库、可回看全部内容及版本的审核台、素材库、发布排期/取消、连接器、人工指标录入与数据复盘、任务队列，以及包含 Prompt 审批/发布/回滚的团队管理与审计。
...
- [x] 后端 76 项测试通过、6 项 PostgreSQL/MinIO 集成测试在本机跳过；真实 PostgreSQL/pgvector 集成测试 3 项、MinIO 集成测试 2 项、新增 1 项 PostgreSQL 限流并发门禁待 CI 执行，分支覆盖率 79.07%，Ruff、锁文件一致性和 `pip-audit` 均通过。
...
- [ ] 持久 PostgreSQL 仍停留在上一 head `c9e7b4a2d610`；Docker 引擎恢复后必须先使用现有回滚点，经 `f4c2d8e7a190 -> a73f9c2e4b61` 迁移到 `b84e0d3f7c92`，并重跑 22 表 PostgreSQL+MinIO 联合恢复。
...
- [ ] 建立 RAG/生成金标评测集，把召回、事实性、安全、PII/版权和成本阈值接入 Prompt/模型发布门禁；当前双人审批不能替代质量评测。
