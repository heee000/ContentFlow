...
先通过测试、备份当前镜像引用/配置、确认队列无运行任务并核验 Serve 未启用 Funnel。建立单独的 `single-operator.env`，包含已确认的测试工作区 UUID，可同时覆盖本次已验真的源 SHA 和镜像引用（均无秘密），保留原配置。使用新的已验证 API/Worker/Web 镜像；后端和前端必须同时更新。
