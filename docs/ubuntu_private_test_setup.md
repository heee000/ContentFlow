...
更新：2026-09-21。范围是用户自有电脑上的私人测试；既有公网/云服务器部署仍暂停。

**当前状态（2026-09-21）**：电脑重启后已通过 Tailscale 恢复连接，六服务自动启动、HTTPS/登录会话和原数据核验通过。r2 在原六例套件中 6/6 通过并按已授权私人单人策略确认/激活；已真实生成 1 篇稿件并停在人工审核，未生成封面或发布。活动名「Ubuntu 内测 0921｜r2｜发布前复核清单」，标题《AI 文案发布前，先过这三道关：事实、承诺、适配》。模型评分 8.2/10 不代替人的质量判断；具体不足与后续检索/引用/修订改进见 CF-20260921-01。以下日期更早的“未评测/未生成”条目均为历史断点。

**最新业务结果（2026-09-19 完整评测后）**：这一轮六次真实模型请求均正常返回，报告 23300 tokens；断言为 5/6 通过，唯一失败是注入用例输出包含禁止回显的标记。没有把连接成功当作评测通过，未生成文章。已准备只补强 plan 非回显规则的 Prompt r2 草稿，尚未真实评测、批准或激活；原套件和 r1 均保留。无输出正文，不能断言模型执行了恶意指令或肯定只是拒绝说明引用。30 项本地测试及 2 子测试通过；下一步是限定验证 r2，不自动重跑旧未知请求。详见 CF-20260919-04；以下旧条目保留为历史记录。

**最新诊断结果（2026-09-19 19:35）**：用户授权的 1 次失败单例请求已成功，约 20.89 秒返回、原断言通过，供应商报告 4279 tokens；账本确认只有 1 条 succeeded，无自动重试。请求输入/模型/Prompt 与历史失败相同，此次未复现错误，仍不能确定历史根因或补算旧账单。旧两个 Eval 仍 error/人工核对，Prompt 草稿和生成门禁不变；单例成功不能替代完整六例评测。详见 CF-20260919-03。
...
2026-09-21 重启后复核：专用 SSH 严格校验通过、Tailscale 在线、六容器运行、Serve 仍为私网；TLS/登录会话与原合成文件 checksum、1024 维索引验证正确。旧 LAN 地址超时不影响 Tailscale 接入，没有改代理/DNS或重新安装。历史两个未知请求仍保留人工核对，本轮新 r2 评测/单稿已按明确授权完成；不再停在“等待授权评测 r2”。
...
现在登录上述私人工作区，点击左侧「2 审核内容」，选择「Ubuntu 内测 0921｜r2｜发布前复核清单」即可查看本轮正文、编辑意见和来源。浏览器重载登录页已实测，但本轮没有代用户完成浏览器登录；API 登录/刷新/退出已验证。若还在登录页，使用原私人账号文件，不需要注册新用户或重新配置 API。

Eval v1 和 Prompt r2 已激活；r1 保持草稿，旧失败记录保留。内容为 needs_review，尚无人工批准。重点检查示例改写、无依据主张与审核过程用语；修改后重新走审核。当前 AI 封面只是一项计划，不必先上传图片才能看文章；内容审核后再选择并验证素材路线。微信渠道未迁移，本轮没有创建微信草稿或公开发布。

### 下一阶段：从部署可访问到真实业务验收

当前盘点（2026-09-21）：1 个管理员成员、r1 draft/r2 active、1 个 active Eval suite、4 个 Eval run（旧 2 error/1 failed，r2 新 1 passed）、1 个活动/1 个待人工审核内容、1 个 planned 图片素材，0 个发布任务。`production`、`require_governed_prompts=true`、`allow_mock_providers=false` 不变，文本/Embedding 均为真实 API。下一步是体验现有稿件，不是重复创建评测/内容。
