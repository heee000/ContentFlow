...
当前盘点（2026-09-19 本轮结束）：1 个管理员成员，2 个 draft Prompt release（r1 原版、r2 非回显候选）、1 个 active Eval suite、3 个 Eval run（2 error 保留人工核对，1 failed 为断言未全通过）；r2 尚无 Eval，活动、内容、渠道均为 0。`production`、`require_governed_prompts=true`、`allow_mock_providers=false` 不变，文本/Embedding 均为真实 API。
